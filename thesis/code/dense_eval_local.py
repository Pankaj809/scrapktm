...
Two encoders, both already cached locally, both small enough for CPU:
  * ``all-MiniLM-L6-v2``                       (22M params, English-centric)
...
Emits:
  thesis/ieee_paper/generated/dense_table.tex   -- LaTeX rows for the paper
  thesis/ieee_paper/generated/dense_analysis.json
"""
...
# --- Force CPU-only, capped threads: keep the machine safe (see module docstring).
...
MODELS = [
    ("all-MiniLM-L6-v2", "MiniLM-L6 (EN)"),
    ("paraphrase-multilingual-MiniLM-L12-v2", "mMiniLM-L12 (ML)"),
]
...
def evaluate(model, queries, chunks, texts, k_max=10):
    doc = SentenceTransformer(model, device="cpu")
    emb = doc.encode(texts, batch_size=16, convert_to_tensor=True,
                     normalize_embeddings=True, show_progress_bar=False)
    qemb = doc.encode([q["query"] for q in queries], batch_size=16,
                      convert_to_tensor=True, normalize_embeddings=True,
                      show_progress_bar=False)
    sims = cos_sim(qemb, emb)  # (nq, nchunks)
    ranked_cids, ranked_dids = [], []
    for i in range(len(queries)):
        order = torch.argsort(sims[i], descending=True)[:k_max].tolist()
        ranked_cids.append([chunks[j]["chunk_id"] for j in order])
        ranked_dids.append([chunks[j]["doc_id"] for j in order])
    del doc, emb, qemb, sims
    return ranked_cids, ranked_dids
...
def main():
    ...
    for model_id, label in MODELS:
        ...
        for cfg, texts in (("raw", raw_texts), ("repaired", rep_texts)):
            cids, dids = evaluate(model_id, queries, chunks, texts)
        ...
        results["models"][model_id] = {"label": label, **row}

    (GEN / "dense_analysis.json").write_text(
        json.dumps(results, indent=2, ensure_ascii=False), encoding="utf-8")

    # LaTeX rows: one per (model, config).
    lines = []
    for model_id, label in MODELS:
